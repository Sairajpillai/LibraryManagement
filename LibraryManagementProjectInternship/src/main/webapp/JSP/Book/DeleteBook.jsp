		...
		<c:when test="${loginResult eq 'success'}">
			<c:choose>
				<c:when test="${message eq 'success'}">
					<h1 style="color: green; text-align: center">BOOK DELETED
						SUCCESSFULLY</h1>
				</c:when>
				<c:when test="${status eq 'failure'}">
					<h1 style="color: red; text-align: center">BOOK DELETION
						FAILED</h1>
				</c:when>
				<c:otherwise>
					<h1 style="color: black; text-align: center">BOOK NOT
...
<c:choose>
	...
	<c:choose>
		<c:when test="${message eq 'success'}">
			<h1 class="success-heading">BOOK DELETED
				SUCCESSFULLY</h1>
		</c:when>
		<c:when test="${status eq 'failure'}">
			<h1 class="failure-heading">BOOK DELETION
				FAILED</h1>
		</c:when>
		<c:otherwise>
			<h1 class="failure-heading">BOOK NOT
				AVAILABLE FOR DELETION</h1>
		</c:otherwise>
	</c:choose>
	</c:when>
