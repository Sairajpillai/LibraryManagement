...
<c:choose>
		<c:when test="${loginResult eq 'success' }">
			<c:choose>
				<c:when test="${bookUpdateStatus eq 'success' }">
					<c:choose>
						<c:when test="${acceptStatus eq 'success' }">
		<h1 class="success-heading">Book Accepted successfully</h1>
		</c:when>
						<c:otherwise>
		<h1 class="failure-heading">Book is either accepted by another Librarian</h1>
		</c:otherwise>
					</c:choose>
				</c:when>
				<c:otherwise>
		...
		<c:otherwise>
               <h1 class="failure-heading"> You are not authorized to request books</h1>
</c:otherwise>
	</c:choose>
