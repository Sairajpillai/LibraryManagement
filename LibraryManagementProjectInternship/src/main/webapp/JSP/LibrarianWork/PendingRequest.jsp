	...
	<c:choose>
		<c:when test="${loginResult eq 'success' }">
			<c:choose>
						...
						<c:when test="${not empty bookRequestList }">
							<c:forEach items="${bookRequestList}" var="book">
								<form action="./librarianwork/approvebook" method="post">
									<table border="1">
										<tr>
										    <th>REQUEST ID</th>
											<th>BOOK ID</th>
										...
										<tr>
											...
											<td><input readonly name="bookid" value="${book.bookid_fk}" /></td>
											...
											<td><input type="hidden" name="reqdate"
												value="${book.reqdate}" />${book.reqdate}</td>
