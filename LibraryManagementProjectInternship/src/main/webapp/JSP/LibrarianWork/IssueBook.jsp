				...
				<c:when test="${status eq 'success' }">
					<c:choose>
						<c:when test="${bookUpdateStatus eq 'success' }">
							<c:choose>
								<c:when test="${approveStatus eq 'success' }">
<h1 class="success-heading">Book Issued to student successfully</h1> 
</c:when>
								<c:otherwise>
<h1 class="failure-heading">Book not issued successfully</h1>
</c:otherwise>
							</c:choose>
						</c:when>
						<c:otherwise>
Book Qty updated
				...
				<c:otherwise>
<h1 class="failure-heading">Book Not available in Stock</h1>
</c:otherwise>
			</c:choose>
